...
module.exports.isValidBoatName = async function isValidBoatName(req, res, next) {
    const prop = stripJS(req.body.name?.trim());
    if (!validator.isAlphaNumeric(prop)) {
        return res.status(400).send('Invalid Input');  // User is authenticated, but not authorized/incorrect info
    }
...
module.exports.isValidBoatType = (req, res, next) => {
    if (req.body.type) {
        const prop = stripJS(req.body.type.trim());
        if (!validator.isAlphaNumeric(prop)) {
            return res.status(400).send('Invalid Input');  // User is authenticated, but not authorized/incorrect info
        }
...
module.exports.isValidBoatLength = (req, res, next) => {
    if (req.body.length) {
        const prop = stripJS(req.body.length);
        if (!validator.isNumeric(prop)) {
            return res.status(400).send('Invalid Input');  // User is authenticated, but not authorized/incorrect info
        }
...
module.exports.isValidBoatPostBody = (req, res, next) => {
    ...
    || !validator.isAlphaNumeric(stripJS(req.body.name)) || !validator.isNumeric(stripJS(req.body.length)) || !validator.isAlphaNumeric(stripJS(req.body.type))) {
...
module.exports.isValidBoatPatchBody = (req, res, next) => {
    if (!req.body.name && !req.body.length && !req.body.type || !validator.isAlphaNumeric(req.body.name)
    || !validator.isNumeric(stripJS(req.body.length)) || !validator.isAlphaNumeric(stripJS(req.body.type))) {
...
module.exports.isValidLoadPostBody = (req, res, next) => {
    ...
    || !validator.isAlphaNumeric(stripJS(req.body.name)) || !validator.isNumeric(stripJS(req.body.length))
    || !validator.isAlphaNumeric(stripJS(req.body.type))) {
        return res.status(400).send('The request object body is invalid');
    }
    next();
}
...
module.exports.isValidLoadPatchBody = (req, res, next) => {
    ...
    if (Object.keys(req.body).length > NUM_LOAD_ATTRIBUTES|| !validator.isAlphaNumeric(stripJS(req.body.name))
    || !validator.isNumeric(stripJS(req.body.length)) || !validator.isAlphaNumeric(stripJS(req.body.type))) {
        return res.status(400).send('The request object body is invalid'); // Too many attributes
    }
    next();
